...
\section{ppE Waveform}\label{section:intro}
\hspace*{15.5pt}Fourier waveform for inspiral phase in standard ppE framework is given by ~\cite{Yunes:2009ke}
\ky{Let's change $\delta\Psi$ to $i \delta\Psi$ and remove $i$ from \eqref{eq:2}. I think this is indeed what you have in (16).}
...
\end{equation}
where \ky{Let's write $u$ in terms of $f$ instead of $\Omega$.}
\begin{equation}
u=(\mathcal{M}_c \Omega)^\frac{1}{3}\,.
\end{equation}
Here $\alpha$, $\beta$, $a$, and $b$ are ppE parameters that capture non-GR effects in the gravitational waveform. $\Omega$ is the orbital angular frequency and $\mathcal{M}_c$ is the chirp mass \ky{Please define chirp mass.}. \ky{Let's explain here $(\alpha,\beta) = (0,0)$ corresponds to the GR case.} In order to obtain the expressions of these parmeteres for different modified gravity theories we consider corrections to the binding energy $E$ and the GW luminosity $\dot{E}$. These corrections lead to the modification of Kepler's law and frequency evolution.\\
\begin{comment}
...
\hspace*{15.5pt}For the conservative correction we need to consider modification to binding energy which can be parametrized as follows\cite{Chatziioannou:2012rf}
 \begin{equation}
 E=E_{GR}\left[1+A \left(\frac{m}{r}\right)^p\right]\,,
 \end{equation}
 \ky{Please write down expression for $E_{GR}$. Since this is just a note, it's ok that you didn't include this, but we'd like to turn this into an actual paper eventually, and a paper should be as self-contained as possible.}
 \ky{Please explain what $A$, $m$, $r$ and $p$ are. Please remember that you need to explain in words or formulas each variables that you introduce in your paper.}
 where we assume A to be small, such that the correction represents a small deformation away from GR. Such a binding energy modifies the Kepler'۪s third law as
 \begin{equation}
 \Omega^2=\frac{m}{r^3} \left[1+\frac{1}{2} A \, p\left(\frac{m}{r}\right)^p\right]\,.
 \end{equation}
  Above equation gives the orbital seperation as \ky{You should pick either $r$ or $a$ for the separation.}
 \begin{equation}\label{eq:g}
 a(t)=a_{GR}\left[1+\frac{1}{6}A\, p\, \eta^{-\frac{2p}{5}}u^{2p}\right]\,.
 \end{equation}
 \ky{Please write down expression for $a_{GR}$.} 
 ...
 \end{equation}
 where $\dot{E}_{GR}$ \ky{Please write down the explicit expression for $\dot{E}_{GR}$. I think Katerina and Nico used slightly weird convention that this $\dot{E}_{GR}$ is proportional to $v^2/r^4$ (which can be obtained by using Kepler's law in GR), but please check their erratum.} is the GR energy flux and the second term is assumed small relative to the first, as we are interested in small deformations away from GR. By invoking energy balance law, Eq.~\eqref{eq:a} gives the frequency evolution as 
 \begin{equation}\label{eq:f}
 \dot{f}=\dot{f}_{GR}\left[1+B\eta^\frac{-2q}{5} u^{2q}+\frac{1}{3}A(2p^2-2p-3)\eta^{\frac{-2p}{5}}u^{2p}\right]\,.
 \end{equation}\label{eq:e}
 \ky{Please write down expression for $\dot f_{GR}$.} 
 
 
 \subsubsection*{Correction to the Amplitude}
 
 \hspace{15.5pt}From the stationary phase approximation\cite{Yunes:2009yz},
...
\end{equation}
where $\dot{f}$ is the frequency evolution of gravitational waves. $\mathcal{A}(t_0)\propto \ddot{Q}$ where $Q$ is the quadruple moment which is proportional to ${a^2(t_0)}$. So we can write

\begin{equation}\label{eq:d}
\mathcal{A}(f)\propto \frac{a^2(t)}{\sqrt{\dot{f}}}\,.
\end{equation}
Using Eqs.~\eqref{eq:f} and \eqref{eq:g} in \eqref{eq:d} and keeping only leading order terms, \ky{When you have negative fractions, let's write it as $-\frac{2q}{5}$ instead of $\frac{-2q}{5}$. Please make this correction throughout.}
...
\end{equation}
When dissipative correction dominates $(q>p)$ \ky{$q < p$? (The dominant one has to be at lower PN order.)}, comparing above equation with the ppE waveform in \eqref{eq:1} we find
\begin{equation}
\alpha=-\frac{1}{2}B\eta^{\frac{-2q}{5}},\hspace*{20pt}a=2q\,.
\end{equation}
When conservative correction dominates $(q<p)$ \ky{$q > p$?},
\begin{equation}
...
\end{equation}
 ...
 \hspace*{15.5pt}Modification to the Fourier phase at the leading order can be calculated as \cite{Chatziioannou:2012rf} \ky{Please explain in more detail how $\delta \Psi$ is derived from $\dot f$.}
 \begin{equation}
 ...
 When dissipative correction dominates $(q>p)$ \ky{Again, I think you mean $q<p$.}, there is a simple relation between $\alpha$ and $\beta$ by eliminating $B$ from Eqs.~(XX) and (XX) \ky{Please fill in.}, which is 
 
\begin{equation}\label{eq:2c}
\alpha=\frac{112}{3}\beta\,.
\end{equation} 

\ky{I guess you can also find a simple relation for the conservative-dominated case right? Although we don't have any concrete example theories for this, let's show this for completeness.}
 
 
 \section*{Alternative}
 
 \ky{In our paper, I suggest to show this alternative approach in the main text as this is simpler, and then we show only important formulas from Keterina and Nico's paper and mention that our alternative approach is consistent with theirs. Then all the remaining equations on PPE can go in the appendix.}
 
 \hspace*{15.5pt}We want to write the orbital separation as
 \begin{equation}
 \label{eq:a-2}
...
\end{equation} 
\ky{Please explain in words the meaning of the variables that you introduce here and in (21).}
Comparing~\eqref{eq:a-2} with \eqref{eq:g},
\begin{equation}
c_{a}=2p \text{,\hspace{20pt}}\gamma_a=\frac{1}{6}\,A\,p\,\eta^{-2p/5}\,.
\end{equation}
...
\ky{Since $\beta$ and $b$ are always the same among the above three cases, I suggest to explain $\beta$ and $b$ first, and then look at the three different cases and present $\alpha$ and $a$ for each case.}
...
\end{equation}
 ...
 \end{equation}
 By comparing Eq.~\eqref{eq:3.3a} with Eq.~\eqref{eq:g} and Eq.~\eqref{eq:3.3b} with Eq.~\eqref{eq:f}, we can obtain $A$, $B$, $p$,  and $q$ for dCS theory. Then using Eqs. \eqref{eq:2a} and \eqref{eq:2b} we can find $\alpha$ and $\beta$ as
