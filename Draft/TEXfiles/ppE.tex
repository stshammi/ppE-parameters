...
\st{Both BHs and NSs possess scalar hair. So I think I should not have a paragraph only for BHs.}\ky{I made the previous paragraph to be about NSs, and the next paragraph to be about BHs.}
...
For scalar-tensor theories containing a single scalar field, such dipole radiation causes non-GR correction to GW phase with ppE paramaeter $\beta$ given by \cite{Jacobson:1999vr,Horbatsch:2011ye,Yunes:2016jcc}
 \begin{equation}
 ...
 \end{equation}
with $a=-2$. One important fact here is that for scalar-tensor theories with time-varying G, dissipative and conservative corrections enter at the same order and one should follow the formalism of Sec.~\ref{gdot} in order to calculate ppE parameters for such theories.
