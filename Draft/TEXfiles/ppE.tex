...
EdGB gravity is a well-known extension of GR. It emerges naturally in the framework of low-energy effective string theories  and it gives one of the simplest viable high-energy modifications to GR \cite{Moura:2006pz,Pani:2009wy}. It also arises as a special case of Horndeski gravity \cite{Zhang:2017unx,Berti:2015itd}. One obtains EdGB action by adding a real scalar field (dilaton) to the Einstein-Hilbert action, where the scalar field is non-minimally coupled to the Gauss-Bonnet term\cite{Kanti:1995vq}.

BHs in EdGB theory are of particular interest since they are fundamentally different from their GR counterparts. Perturbative EdGB BH solution is available for slowly rotating BHs and also an exact solution exists for static BH, while Rapidly rotating BH solutions have been found numerically as well \cite{Berti:2015itd,Kanti:1995vq,Kleihaus:2011tg,Pani:2011gy}. Compared to the BHs in GR, there are signficant differences in the ISCO location and orbital frequency of EdGB BHs , which can be detected through astrophysical observations\cite{Pani:2009wy} \st{I am not sure if these properties are related to GW observations.}.
...
One of the prominant non-GR characteristics of EdGB BHs is that they have scalar monopole charge which can generate dipole radiation which in turn changes the speed of binary inspiral. This effect can modify the GW phase and such modification is given by ppE parameter $\beta$ as a function of system parameters as \cite{Yunes:2016jcc,Yagi:2011xp}
\begin{equation}
 \beta_{\text{EdGB}}=\frac{-5}{7168}\zeta_{\text{EdGB}}\frac{(m_1^2s_2^{\text{EdGB}}-m_2^2s_1^{\text{EdGB}})^2}{m^4\eta^{\frac{18}{5}}}\,.
 \end{equation}
 Here, $\zeta_{\text{EdGB}}$ is the dimensionless EdGB coupling paramater which is given by $\zeta_{\text{EdGB}}=16 \pi \alpha^2/m^4$ \cite{Yunes:2016jcc}. \st{I feel like I need to explain what $\alpha$ is in EdGB theory. But I am not sure and we can talk about it in the meeting}. $s_{A}^{\text{EdGB}}$ are the spin-dependent factors of the BH scalar charges which are given by $s_{A}^{\text{EdGB}}\equiv 2(\sqrt{1-{\chi_A}^2}-1+{\chi_A}^2)/{\chi_A}^2~$\cite{Berti:2018cxi,Prabhu:2018aun}. Here, $\chi_A$ is the magnitude of the spin angular momentum of the $\mathit{A}\text{th}$ body normalized by its mass squared. In EdGB theory the correction to the phase enters through the correction of GW eneregy flux hence it's a dissipative-dominated case, which means we can use Eq. \eqref{eq:2w2} to calculate ppE parameter $\alpha_{\text{EdGB}}$ from $\beta_{\text{EdGB}}$ as  
 \begin{equation}
