...
% significantly improve upon solar system constraints, and probe gravity in a specific sector of strong-field which is dynamical but quasi-linear 
\cite{Freire:2012mg,Kramer:2006nb,Liu:2011ae,Ransom:2014xla,Stairs:2002cw,Stairs:1997kz,Stairs:2003eg,taylor1992experimental,Wex:2014nva,Yunes:2013dva}. 
...
Until now, six GW sources have been found (five of them are from binary black holes \cite{Abbott:2016blz,Abbott:2016nmj,Abbott:2017vtc,Abbott:2017gyy,Abbott:2017oio} and the remaining one is from a binary neutron star \cite{TheLIGOScientific:2017qsa}), which opened a completely new way of testing GR. 
...
A viable modified theory of gravitation reduces to GR in weak-field limit, which makes it reasonable to search for the evidence of such theory in nondynamical strong fields. GWs emitted from binary stellar and black hole mergers are the most promising tools for that purpose.  Analysis of the events GW150914 \cite{Abbott:2016blz} and GW151226 \cite{Abbott:2016nmj} placed constriants on several mechanisms associated with alternative theories of gravity \cite{Yunes:2016jcc}. Recently, constraints on the models of dark energy have been calculated from GW170817 as well \cite{Battye:2018ssx,Ezquiaga:2018btd}. 
\ky{I'll edit this paragraph later after I see what you've written in the subsequent two paragraphs to minimize the overlap.}

%3 propagation and generation mechanisms,polarizations,ppE formalism
Non-GR corrections can eneter in the GW waveform through activation of different theoretical mechanisms, which can be classified as generation mechanisms and propagation mechanisms \cite{Yunes:2016jcc}. Generation mechanisms take place close to the source (binary), while propagation mechanisms occur in the far-zone and accumulate as the waves propagate. In our papar we focus on the corrections which originate from generation mechanisms. Modifications that happen at the stage of propagation of GWs can be found in Refs.~\cite{Mirshekari:2011yq,Yunes:2016jcc,Nishizawa:2017nef}. While a generic metric theory of gravity permits six different polarization modes of gravitational wave, only two of them are physical in GR which are the quadrupular tensor modes. The additional modes include one monopolar transverse mode called ``breathing" mode, and three longitudinal vector modes \cite{PhysRevD.8.3308,Will:2005va}. We follow the standard parametrized post-Einsteinian (ppE) framework \cite{Cornish:2011ys,Yunes:2009ke}, ignoring the additional non-GR modes whose effects enter in higher post-Newtonian (PN) order. A more generalized framework considering all six polarizations can be found in Ref. \cite{Chatziioannou:2012rf}
%ppE framework


%Our calculation
Generic mappings among ppE phase corrections and amplitude corrections have been derived previously in Ref. \cite{Chatziioannou:2012rf}, depending on whether the dominant correction orginates from dissipative or conservative sector. Such mappingss allow us to obtain non-GR deviations in amplitude directly from the non-GR deviations in phase. In our paper we rederive those mappings using a different choice of parametrization, which significantly simplifies the previously derived results. The original formalism which is summarized in App.~\ref{appendix}, consider corrections to binding energy and rate of change of binding energy of the binary and express the ppE parameters in terms of those corrections. We rather choose corrections to frequency evolution and orbital seperation as our variables, and express ppE parameters in terms of those. We then consider a collection of example theories and show the ppE amplitude and phase corrections as functions of representative parameters of each theory, as summarized in Table \ref{table:ppE}.

%Varying-G theory(An exception of our formalism)
...
\hline
\noalign{\smallskip}
\multirow{2}{*}{Theories}&\multicolumn{4}{c}{ppE Parameters}\\ 
&  $\beta$ & $b$ & $\alpha$   & $a$  \\ \hline
Scalar-Tensor Theories & $-\frac{5}{7168}\eta ^{2/5}(\alpha_1-\alpha_2)^2$&-7&$-\frac{5}{192}\eta ^{2/5}(\alpha_1-\alpha_2)^2$&-2\\ \hline
EdGB Gravity&$-\frac{5}{7168}\zeta_\EDGB\frac{(m_1^2\tilde s_2^\EDGB-m_2^2\tilde s_1^\EDGB)^2}{m^4\eta^{18/5}}$&-7&$-\frac{5}{192}\zeta_\EDGB\frac{(m_1^2 \tilde s_2^\EDGB-m_2^2 \tilde s_1^\EDGB)^2}{m^4\eta^{18/5}}$&-2\\ \hline
dCS Gravity& $\begin{aligned}
&\frac{1549225}{11812864}\eta ^{-14/5} \zeta_{\DCS} \left[-2 \delta_m \chi_a \chi_s \right.\nonumber\\ 
 &\left. +\left(1-\frac{16068 \eta }{61969}\right)\chi_a^2 +\left(1-\frac{231808 \eta }{61969}\right)\chi_s^2\right]
\end{aligned} $ &-1& $\begin{aligned}
&\frac{185627}{1107456}\eta ^{-14/5}\zeta_{\DCS} \left[-2 \delta_m \chi_a \chi_s \right. \nonumber\\ 
 &\left. +\left(1-\frac{53408 \eta }{14279}\right) \chi_a^2+\left(1-\frac{3708 \eta }{14279}\right) \chi_s^2\right]
\end{aligned} $&4\\ \hline
Einstein-\AE ther Theory& $\begin{aligned}
&-\frac{5}{3584}\eta ^{2/5} (s_1^\EA-s_2^\EA)^2\nonumber\\
 & \times \frac{(c_{14}-2) w_0^3-w_1^3}{c_{14} w_0^3 w_1^3 [(1-s_1^\EA) (1-s_2^\EA)]^{4/3}}
\end{aligned}$  & -7 & $\begin{aligned}
&-\frac{5}{96}\eta ^{2/5} (s_1^\EA-s_2^\EA)^2 \nonumber\\
 & \times  \frac{(c_{14}-2) w_0^3-w_1^3}{c_{14} w_0^3 w_1^3 [ (1-s_1^\EA) (1-s_2^\EA)]^{4/3}}
\end{aligned}$& -2\\ \hline
Khronometric Theory&$\begin{aligned}
&-\frac{5}{96} \eta ^{2/5}\sqrt{\bar \alpha_{\KG}}\frac{(s_1^\KG-s_2^\KG)^2}{[(1-s_1^\KG) (1-s_2^\KG)]^{4/3}} \nonumber\\
 & \times \left[\frac{(\bar \beta_{\KG}-1)(2+\bar \beta_{\KG}+3\bar \lambda_{\KG})}{(\bar \alpha_{\KG}-2)(\bar \beta_{\KG}+\bar \lambda_{\KG})}\right]^{3/2}
\end{aligned}$&-7&$\begin{aligned}
&-\frac{5}{96} \eta ^{2/5}\sqrt{\bar \alpha_{\KG}}\frac{(s_1^\KG-s_2^\KG)^2}{[(1-s_1^\KG) (1-s_2^\KG)]^{4/3}} \nonumber\\
 & \times \left[\frac{(\bar \beta_{\KG}-1)(2+\bar \beta_{\KG}+3\bar \lambda_{\KG})}{(\bar \alpha_{\KG}-2)(\bar \beta_{\KG}+\bar \lambda_{\KG})}\right]^{3/2}\
\end{aligned}$&-2 \\ \hline 
Noncommutative Gravity & $-\frac{75}{256}\eta ^{-4/5}(2 \eta -1) \Lambda ^2$ &-1& $-\frac{3}{8}\eta ^{-4/5}(2 \eta -1) \Lambda ^2$ &4\\ \hline
Varying-G Theories&$-\frac{75 \mathcal{M}_0}{851968 {G_0}^{10/3}}\left(\frac{11 \dot{G}_0}{3 G_0}+ \frac{17 \dot{m}_0}{3m_0}- \frac{m_{1,0}\dot{m}_{2,0}+\dot{m}_{1,0}m_{2,0}}{{m_0}^2 \eta0}\right)$& -13 & $-\frac{5\mathcal{M}_0}{512 {G_0}^{5/3}} \left(\frac{7 \dot{G}_0}{ G_0} + \frac{5\dot{m}_0}{m_0}+\frac{m_{1,0}\dot{m}_{2,0}+\dot{m}_{1,0}m_{2,0}}{{m_0}^2 \eta0}\right)$ &-8\\ 
\noalign{\smallskip}
\hline
\hline
\end{tabular}
\end{centering}
\caption{ppE parameters in different modified theories of gravity. $\beta$ and $b$ give the non-GR corrections to GW phase, while $\alpha$ and $a$ give the non-GR corrections to GW amplitude in Fourier space.}
\label{table:ppE}
\end{table*}
\endgroup
}
