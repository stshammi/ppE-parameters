...
\ky{Amplitude corrections to ST theories have already been derived in some literature.}
}
\label{table:ppE_amp}
\end{table*}
\endgroup
...
Using Eqs.~\eqref{eq:3.7a2},~\eqref{eq:3.7a3}, and~\eqref{eq:3.7a4} in Eq.~\eqref{eq:3.7d} and keeping only leading order correction terms, we can write the amplitude in Fourier space as
\begin{equation}
...
\begin{equation}\label{eq:3.7d2}
 ...
 \end{equation}
\st{I added this paragraph and the next one}. PPE parameters in Eqs.~\eqref{eq:beta_gdot} and~\eqref{eq:3.7d2} do not match with the previous work on varying-G theories in Ref.~\cite{Yunes:2009bv}. The discrepancy arises for two reasons: the masses of the binary components were kept constant in~\cite{Yunes:2009bv}, and a term was missing in the energy-balance equation. The assumption of constant mass is not applicable for compact stellar binaries, where the gravitational self-energy is not negligible. The second reason is explained in App.~\ref{appendix_2}, where we derived the GW frequency evolution from energy-balance law. \st{I still do not know if the first argument applies for black holes, since the binding energy of a black hole does not really make sense. I have to think about it. Also I need to contact Nico to make sure whether Brans-Dicke theory is varying-G theory or not. If BD theory has time-varying G, then at least we have one example varying-G theory where mass of a black hole is also a variable. For now I keep it as `compact stellar binary'}
