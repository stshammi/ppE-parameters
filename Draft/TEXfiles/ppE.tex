...
dCS gravity is described by Einstein-Hilbert action with a dynamical (pseudo) scalar field which is non-minimally coupled to the Pontryagin densiy or Chern-Simons (CS) term with a coupling constant $\bar{\alpha}_{\DCS}$\cite{Alexander:2009tp,Jackiw:2003pm}. Similar to EdGB gravity, dCS gravity can be obtained as an effective field theory from the compactification of  heterotic string theory \cite{GREEN1984117,McNees:2015srl}. It is also important in the context of particle physics \cite{Alexander:2009tp,Mariz:2004cv,MARIZ2008312,PhysRevD.78.025029}, loop quantum gravity  \cite{PhysRevD.80.104007,Taveras:2008yf}, and inflationary cosmology \cite{Weinberg:2008hq}. Considering a cut-off scale for the energy, the upper bound of the coupling constant $\bar{\alpha}_{\DCS}$ is determined by table-top experiment as $\sqrt{|\bar{\alpha}_{\DCS}|} < \mathcal{O}(10^8 \text{km})$ \cite{Yagi:2012ya}. Same constraint has been found from the measurement of frame-dragging effect by Gravity Probe B and LAGEOS satellites \cite{AliHaimoud:2011fw}.

dCS gravity reduces to GR in weak-field limit, hence stringent constraint cannot be obtained from weak-field experiments \cite{Yagi:2012vf}. A large-curvature environment is required to distinguish the theory from GR which makes binary black hole mergers a promising sector for this purpose. Also, dCS gravity only predicts correction in the presence of parity-odd sources \cite{Alexander:2009tp,Berti:2015itd} and only observation of GWs from rotating BHs could yield significant correction. A projected bound has been derived in ref. \cite{Yagi:2012vf} considering favorable spin-orientations of BBH mergers which gives six to seven order of magnitude stronger constraint than the bound obtained from solar system experiments.

We now derive the expressions of ppE parameters for dCS gravity. While EdGB BHs have scalar monopole charge, BHs in dCS gravity contain scalar dipole charge which induces modified quadrupolar emission \cite{Yagi:2011xp,Yunes:2016jcc}. This modification results in dissipative and conservative corrections at the same order. Conservative correction yields the correction to Kepler's law as $r=(m\eta^{2/5}/u^2)(1+\delta C_r \eta^{-4/5}u^4)$, comparing with Eq.~\eqref{eq:2k} which gives the amplitude correction to binary seperation as
\begin{equation}\label{dcs:gamma_r}
\gamma_r=\eta^{-4/5}\delta C_r\,,
\end{equation}
with exponent correction $c_r=4$, where $\delta C_r$ for spin-aligned binaries is given by \cite{Yagi:2012vf}
\begin{align}
\delta C_r=\frac{25}{256}\zeta_{\DCS} \frac{\chi_1 \chi_2}{\eta}-\frac{201}{3584}\zeta_{\DCS} m^2\left(\frac{{\chi_1}^2}{{m_1}^2}+\frac{{\chi_2}^2}{{m_2}^2}\right)\,.
\end{align} 

 \ky{I think we can just show $\gamma_r$ directly instead of (38) and (39).}\st{OK, done.}
 
 %\begin{equation}\label{eq:3.3a}
 %a=\frac{m}{u^2}(1+\delta C_r v^4)\,,
 %\end{equation}
 
 \ky{Let's change $\zeta m^2 \left(\frac{{\chi_1}^2}{{m_1}^2}+\frac{{\chi_2}^2}{{m_2}^2}\right)$ to $\frac{\zeta}{\eta^2}\left(\frac{m_2}{m_1}\chi_1^2+\frac{m_1}{m_2}\chi_2^2\right)$ so that each piece is dimensionless. Please do the same for (41).}
\st{I actually don't get  $\frac{\zeta}{\eta^2}\left(\frac{m_2}{m_1}\chi_1^2+\frac{m_1}{m_2}\chi_2^2\right)$ if I do the calculation, let's talk about it in the meeting...........}
Here, $m_A$ is the individual mass, $\chi_A =\left | S_{A}^{i} \right |/m_A$ is the dimensionless Kerr spin parameter where $S_{A}^{i}$ is the spin angular momentum vector, all relative to the $\mathit{A}\text{th}$ BH. The magnitude of the correction to the binary inspiral rate is proportional to the dimensionless dCS coupling parameter $\zeta_{\DCS}=16\pi \bar{\alpha}_{\text{dCS}}^2/m^4$.

Dissipative correction gives frequency evolution as $\dot{f}=\dot{f}_{\GR}(1+\delta C \eta^{-4/5}u^4)$, comparing with Eq. \eqref{eq:2m} which gives
\begin{equation}\label{dcs:gamma_fdot}
\gamma_{\dot{f}}=\eta^{-4/5}\delta C\,,
\end{equation}
%
...
%\end{equation}
%
with $c_{\dot{f}}=4$, where $\delta C$ is given by \cite{Yagi:2012vf}
 \begin{equation}
 \delta C= \frac{38525}{39552 }\zeta_{\DCS}\frac{\chi_1 \chi_2}{\eta }-\frac{309845}{553728 }\zeta_{\DCS} m^2\left(\frac{{\chi_1}^2}{{m_1}^2}+\frac{{\chi_2}^2}{{m_2}^2}\right)\,.
 \end{equation}
 Using Eqs. \eqref{dcs:gamma_r} and \eqref{dcs:gamma_fdot} in Eqs.~\eqref{eq:2u} and \eqref{eq:2v} respectively, one obtains
 
 \begin{align}
 \alpha_{\DCS}=\frac{185627}{1107456}\eta ^{-14/5}\zeta_{\DCS} \left[-2 \delta_m \chi_a \chi_s+\left(1-\frac{53408 \eta }{14279}\right) \chi_a^2\right. \nonumber\\ \left.+\left(1-\frac{3708 \eta }{14279}\right) \chi_s^2\right]\,,
 \end{align}
with $a=4$, and
 \begin{align}
 \beta_{\DCS}=\frac{1549225}{11812864}\eta ^{-14/5} \zeta_{\DCS} \left[-2 \delta_m \chi_a \chi_s+\left(1-\frac{16068 \eta }{61969}\right)\chi_a^2 \right.\nonumber\\ \left.+\left(1-\frac{231808 \eta }{61969}\right)\chi_s^2\right]\,,
 \end{align}
with $b=-1$. Here, $\chi_{s,a}=(\frac{a_{||1}}{m_1}\pm\frac{a_{||2}}{m_2})/2$  are the symmetric and antisymmetric dimensionless spin combinations\footnote{$\chi_s$ is different from $\chi_{\text{eff}}\equiv (a_{||1}+a_{||2})/m$ in \cite{TheLIGOScientific:2016wfe}} with $a_{||A}$ representing the projection of the (dimensional) spin vector $\vec{a}_A$ onto the unit orbital angular momentum vector, and $\delta_m=(m_1-m_2)/m$ is the dimensionless mass difference.
