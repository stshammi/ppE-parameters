...
\end{equation}
as~\cite{PhysRevLett.65.953} \ky{I think you're missing the subscript 0 in $G_C$ and $\dot G_C$. Please make sure to fix this throughout.}
...
First, GW emission makes the orbital separation $r$ decay with the rate given by~\cite{PhysRevD.49.2658}
 ...
 \end{equation}
Second, time variation of mass, gravitational constant and specific angular momentum changes $r$ at a rate of \ky{Aren't you missing $r$ on the rhs?}
 \begin{equation}
 ...
 \end{equation}
Using the evolution of the binary separation $\dot{r}\equiv \dot{r}_{\GW}+\dot{r}_H$, together with Eqs.~\eqref{eq:3.7a4-3} and~\eqref{eq:3.7a4-4}, in Eq.~\eqref{eq:3.7a}, we can find the GW frequency evolution as \ky{The purpose of this paper is to derive PPE parameters and for that, we don't need to present expressions valid to 2PN. I removed higher order PN terms.}
%\bw
...
\end{eqnarray}
where $x=v^2=(\pi G_C  m f)^{2/3}$ \ky{I included $G_C$ in the definition of $x$ and removed unnecessary $G_C$'s throughout.} is the squared velocity of the relative motion. Here we only considered the leading correction to the frequency evolution entering at -4PN order. 
\ky{Let's show here the expression for $\dot f$ where we've substituted Eqs.(48)-(50). Otherwise, the below sentences don't make sense since $G_{C,0}$ and $G_{D,0}$ don't appear in the above expression yet...}
...
We integrate Eq.~\eqref{eq:3.7b} to obtain time before coalescence $t(f)-t_0$ and the GW phase $\phi(f)\equiv \int 2 \pi f dt = \int (2\pi f/\dot{f})df$ as
...
&& \left. \left. -41 (m_{1,0}s_{1,0}+m_{2,0}s_{2,0})\right]x_0^{-4} \right\}\,,
\end{eqnarray}
with $u_0 \equiv (\pi G_0 \mathcal{M}_0 f)^\frac{1}{3}$ \ky{I included $G_0$ in the def of $u_0$ and removed unnecessary $G_0$'s throughout.}
...
\begin{align}\label{eq:3.7c}
\Psi(f)=&2\pi ft(f)-\phi(f)-\frac{\pi}{4}\nonumber\\
=&2\pi f t_0-\phi_0-\frac{\pi}{4}+\frac{3}{128}{G_0}^{-5/3}u_0^{-5}\left\{1-\frac{25}{19968}\frac{\dot{G}_{C,0}}{\eta_0}\left[11m_0+3(s_{1,0}+s_{2,0}-\delta)m_0-41 (m_{1,0}s_{1,0}+m_{2,0}s_{2,0})\right]x_0^{-4} \right\}\,.
\end{align}
\ew
From Eq.~\eqref{eq:3.7c}, one finds the PPE phase parameters as $b=-13$ and \ky{I think $11m_0$ should be $14m_0$. If you agree, please also correct $t$, $\phi$ and $\Psi$ above.}
...
Using furhter Eqs.~\eqref{eq:3.7a2}-\eqref{eq:3.7a4-2} in Eq.~\eqref{eq:3.7d}, we find the amplitude PPE parameters as $a = -8$ and \ky{I think $-7m_0$ should be $-6m_0$.}
%\begin{equation}
