...
Scalar-tensor theories of gravity are some of the most well-eshtablished modified gravity theories where one or more scalar field(s) are introduced through non-minimal coupling, i.e., coupling of Ricci scalar in Einstein-Hilbert action with a function of scalar field(s)\cite{Berti:2015itd,Chiba:1997ms,PhysRevD.6.2077}. Such theories arise naturally as the dimensionally reduced effective theories of higher dimensional theories, like Kaluza-Klein \cite{Fujii:2003pa,Overduin:1998pn} and string theories \cite{Fujii:2003pa,Fujii:2004bg}, and also appear in brane-world scenario \cite{Randall:1999vf,Randall:1999ee}. Scalar-tensor theories have implications in cosmology as well since they are viable candidate for explaining dark-energy \cite{Brax:2004qh,PhysRevD.73.083510,PhysRevD.62.123510,PhysRevD.66.023525,Schimd:2004nq}, structure formation \cite{Brax:2005ew}, inflation \cite{Burd:1991ns,Barrow:1990nv,Banerjee:1993ct,Clifton:2011jh}, and primordial nucleosynthesis \cite{Coc:2006rt,Damour:1998ae,Larena:2005tu,Torres:1995je}. They also offer simple ways to self-consistently model possible variations in Newton's constant \cite{Clifton:2011jh}. A protoype scalar-tensor theory is Brans-Dicke theory, where a scalar gravitational field is postulated along with the metric, with an effective strength inversely proportional to the coupling parameter $\omega_{\BD}$ \cite{PhysRev.124.925,Scharre:2001hn}. So far the most stringent bound on $\omega_{\BD}$ has been found from Cassini-Huygens experiment on Shappiro time delay, which gives $\omega_{\BD}>4\times10^4$ \cite{Bertotti:2003rm,Zhang:2017sym}. Another extensively studied scalar-tensor theory is mono-scalar-tensor theory of Damour-Esposito-Far\`ese (DEF) gravity, which predicts nonperturbative scalarization phenomena for neutron stars \cite{Barausse:2012da,PhysRevLett.70.2220}. By combining the observational results from multiple pulsar systems and applying Markov-chain Monte Carlo simulations, the strongest upper bound on the scalarization parameter ($\beta_0$) has been obtained as $\beta_0\gtrsim -4.28$ at $68\%$ confidence level and $\beta_0\gtrsim -4.38$ at $90\%$ confidence level \cite{Shao:2017gwu}.
...
\st{Both BHs and NSs possess scalar hair. So I think I should not have a paragraph only for BHs.}
So how the BHs in scalar-tensor theories are different from GR BHs? One prominent non-GR feature of Scalar-tensor BHs is that they can evade no-hair theorem and acquire scalar hair supported by time-dependent boundary conditions \cite{Berti:2013gfa}. Such time-dependence could arise either from cosmological evolution, or due to the slow motion of the black hole within the asymptotic spatial gradient in the scalar field \cite{Horbatsch:2011ye}. Because of the scalar charge, inspiriling binary black holes can emit dipole scalar radiation, given that the mass of the scalar particle is smaller than the orbital period ($m \ll \Omega$)\cite{Horbatsch:2011ye}. For scalar-tensor theories containing a single scalar field, such dipole radiation causes non-GR correction to GW phase with ppE paramaeter $\beta$ given by \cite{Jacobson:1999vr,Horbatsch:2011ye,Yunes:2016jcc}
 \begin{equation}
 \beta_{\ST}=-\frac{5}{1792}\dot{\phi}^2\eta^{\frac{2}{5}}(m_1s_1^{\ST}-m_2s_2^{\ST})^2\,,
 \end{equation}
 with $b=-7$. Here, $\dot{\phi}$ is the growth rate of scalar field, $m_A$ is the individual mass, and $s_A^{\ST}\equiv[1+(1-\chi_A^2)^{1/2}]/2$ are the spin-dependent factors of BH scalar charges \cite{Horbatsch:2011ye} where $\chi_A$ is the magnitude of the spin angular momentum of the $\mathit{A}\text{th}$ body normalized by its mass squared. Leading order correction in scalar-tensor theory is of dissipative kind, hence using Eq. \eqref{eq:2w2} we can write the ppE amplitude correction as
 
 \begin{equation}
 \alpha_{\ST}=-\frac{5}{48}\dot{\phi}^2\eta^{\frac{2}{5}}(m_1s_1^{\ST}-m_2s_2^{\ST})^2\,,
 \end{equation}
with $a=-2$. One important fact here is that for scalar-tensor theories with time-varying G, dissipative and conservative corrections enter at the same order and one should follow the formalism of \ref{gdot} in order to calculate ppE parameters for such theories.
...
&-\frac{309845}{553728 }\eta^{-14/5}\zeta_{\DCS} \left(\frac{m_1^2}{m^2}\chi_2^2+\frac{m_2^2}{m^2}\chi_1^2\right)\,,
 \end{align}
with $c_{\dot{f}}=4$. Here $\zeta_{\DCS}=16\pi \bar{\alpha}_{\text{dCS}}^2/m^4$ is the dimensionless coupling constant. \ky{I commented out the explanation for $\chi_A$ since we already explain it in the EdGB section. The one for $m_A$ should go in the ST section.}
