...
\end{tabular}
\end{centering}
\caption{PPE corrections to GW phase in various modified theories of gravity. PPE gravitational waveform in Fourier space is given by $\tilde{h}(f)\equiv\tilde{h}_{\GR}(1+\alpha u^a)e^{i\delta\Psi}$ with $u\equiv(\pi \mathcal{M}f)^{1/3}$, where $\tilde{h}_{\GR}$ is the gravitational waveform in GR. $\mathcal{M}\equiv(m_1m_2)^{3/5}/(m_1+m_2)^{1/5}$ is the chirp mass and $\eta\equiv(m_1m_2/m^2)$ is the symmetric mass ratio of the binary with component masses $m_1$ and $m_2$ and total mass $m\equiv m_1+m_2$. Non-GR correction to GW phase is $\delta\Psi\equiv\beta u^b$, where $\beta$ is the magnitude correction (second coulmn) and $b$ is the exponent correction (third column). The expressions in boldface are derived in this paper for the first time. The mass, sensitivity, and scalar charge of the $A$th binary component are reperesented by $m_A$, $s_A$, and $\alpha_A$ respectively. $\alpha_A$ depend on specific theories and compact objects, while sensitivities $s_A$ roughly equal to the compactness of the bodies (0.5 for BHs and $\sim 0.2$ for NSs). $\zeta_\EDGB\equiv 16 \pi \bar{\alpha}_\EDGB^2/m^4$ and $\zeta_{\DCS}\equiv 16\pi \bar{\alpha}_{\text{dCS}}^2/m^4$ are the dimensionless coupling constants in EdGB and dCS gravity respectively. $\tilde s_{A}^\EDGB$ are the spin-dependent factors of the BH scalar charges in EdGB gravity given by $\tilde s_{A}^\EDGB\equiv 2(\sqrt{1-{\chi_A}^2}-1+{\chi_A}^2)/{\chi_A}^2~$, where $\chi_A$ is the magnitude of the spin angular momentum of the $\mathit{A}\text{th}$ body normalized by its mass squared. $\chi_{s,a}\equiv(\chi_1 \pm \chi_2 )/2$ are the symmetric and antisymmetric combinations of dimensionless spin parameters and $\delta_m\equiv(m_1-m_2)/m$ is the fractional difference in masses relative to the total mass. Einstein-\AE ther theory and khronometric gravity are the Lorentz violating theories where the amount of Lorentz violation is controlled by $(c_1,c_2,c_3,c_4)$ and $(\bar \alpha_{\KG}, \bar \beta_{\KG}, \bar \lambda_{\KG})$ respectively. $\omega_s$ is the propagation speed of spin-s mode in EA theory given by Eqs.~\eqref{eq:Prop_Speed_EA_1}-\eqref{eq:Prop_Speed_EA_3}, and $c_{14}\equiv c_1+c_4$. The representative parameter in noncommutative gravity is $\Lambda^2\equiv\theta^{0i} \theta_{0i}/(l_p^2 t_p^2)$, where $l_p$ and $t_p$ are the Planck length and time respectively. $\theta^{0i}$ are the time-spatial components of a real constant antisymmetric tensor controlling the quantum fuzziness of the spacetime coordinates. Subscript $0$ in varying-G theories denotes the quantity measured at time $t=t_0$ with $t_0$ being the time of coalescence. A dot over a quantity is simply the time derivative of the quantity within the approximation of no spatial variation.
}
...
Scalar-Tensor Theories &$\mathbf{-\frac{5}{192}\eta ^{2/5}(\alpha_1-\alpha_2)^2}$&$\mathbf{-2}$\\ \hline
...
$&$\mathbf{4}$\\ \hline
Einstein-\AE ther Theory~\cite{Hansen:2014ewa}& $\mathbf{-\frac{5}{96}\eta ^{2/5} (s_1^\EA-s_2^\EA)^2\frac{(c_{14}-2) w_0^3-w_1^3}{c_{14} w_0^3 w_1^3 [ (1-s_1^\EA) (1-s_2^\EA)]^{4/3}}}$& $\mathbf{-2}$\\ \hline
Khronometric Theory~\cite{Hansen:2014ewa}&$\mathbf{-\frac{5}{96} \eta ^{2/5}\sqrt{\bar \alpha_{\KG}}\frac{(s_1^\KG-s_2^\KG)^2}{[(1-s_1^\KG) (1-s_2^\KG)]^{4/3}} \left[\frac{(\bar \beta_{\KG}-1)(2+\bar \beta_{\KG}+3\bar \lambda_{\KG})}{(\bar \alpha_{\KG}-2)(\bar \beta_{\KG}+\bar \lambda_{\KG})}\right]^{3/2}}$&$\mathbf{-2}$ \\ \hline 
...
\end{tabular}
\end{centering}
\caption{PPE corrections to GW amplitude in various modified theories of gravity. PPE waveform in Fourier space is given by $\tilde{h}(f)\equiv\tilde{h}_{\GR}(1+\alpha u^a)e^{i\delta\Psi}$, where non-GR correction to the amplitude is parametrized by magnitude $\alpha$ (second column)and exponent $a$ (third column). All other terms have same meanings as Table~\ref{table:ppE_phase}.}
\label{table:ppE_amp}
\end{table*}
\endgroup
}
